...
def figure_move(t, kind):
    if kind == 'heart':
        ...
        x_current = np.sin(t) * (np.exp(np.cos(t)) - 2 * np.cos(4 * t) + np.sin(t / 12) ** 5)
        y_current = np.cos(t) * (np.exp(np.cos(t)) - 2 * np.cos(4 * t) + np.sin(t / 12) ** 5)
        return x_current, y_current
...
if kind == 'butterfly':
    ani = FuncAnimation(fig, butterfly_animate, frames=np.arange(0, 12 * np.pi, 0.1), interval=30)
elif kind == 'heart':
    ani = FuncAnimation(fig, heart_animate, frames=np.arange(0, 2 * np.pi, 0.1), interval=30 )
